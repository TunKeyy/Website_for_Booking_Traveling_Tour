...
<link rel="stylesheet" href="/front/css/offers.css">
    ...
    <div class="best-offers">
        ...
        <div class="offers-grid">
            @foreach ($search_tours as $s_tour)
                <div class="grid1 all-grid">
                    <div class="box1">
                        <img src="/front/images/tour_images/{{ $s_tour->image }}" alt="phuquoc">
                        <div class="offer-title">
                            {{ $s_tour->destination }}
                        </div>
                    </div>
                    <div class="box2">
                        <div class="offers_price">{{ number_format($s_tour->price) }} VNĐ<span>10 days</span></div>
                        <div class="star">
                        ...
                        <div class="bookingForm">
                            ...
                            <form id="bookingForm" name="bookingForm"
                                action="{{ route('BookingForm.storeSearch', ['id' => $s_tour->id]) }}" method="POST">
                                <input type="hidden" name="_token" value="{{csrf_token()}}">
                                <h1>THÔNG TIN LIÊN HỆ</h1>
                                <div class="bookTourInfo">
                                    <img src="{{ asset('front/images/tour_images') }}/{{ $s_tour->image }}"
                                        alt="">
                                    ...
                                    <div class="bookTourInfo_detail">
                                        <p id="bookTourInfo_detail_code">Mã tour: {{ $s_tour->id }}</p>
                                        {{-- <p id="bookTourInfo_detail_departure">Nơi xuất phát: Lấy từ Database</p> --}}
                                        <p id="bookTourInfo_detail_destination">Điểm đến: {{ $s_tour->destination }}</p>
                                        <p id="bookTourInfo_detail_Dateofdepartment">Ngày xuất phát:
                                            {{ $s_tour->start_date }}</p>
                                        <p id="bookTourInfo_detail_ArrivalDate">Ngày đến: {{ $s_tour->back_date }}</p>
                                        <p id="bookTourInfo_detail_price">Giá: {{ number_format($s_tour->price) }} VNĐ
                                        </p>
                                    </div>
                                </div>
